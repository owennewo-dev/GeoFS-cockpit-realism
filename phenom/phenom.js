...
function destroy() {
    ...
    try { if (window.geofsAddonRefreshWaypoints === window.geofsPhenomAddon.reloadWaypoints) delete window.geofsAddonRefreshWaypoints; } catch(e) { }
    try { if (window.geofsAddonToggleTerrain === window.geofsPhenomAddon.toggleTerrain) delete window.geofsAddonToggleTerrain; } catch(e) { }
}
...
window.geofsPhenomAddon.toggleTerrain = function() {
    try{
        if (map && map.getLayer && map.getLayer('hills')){
            ...
            console.log('[Phenom] Terrain layer visibility set to', next);
        } else {
            console.warn('[Phenom] Terrain layer not present yet');
        }
    }catch(e){ console.error('[Phenom] Toggle terrain error:', e); }
};
window.geofsAddonToggleTerrain = window.geofsPhenomAddon.toggleTerrain;